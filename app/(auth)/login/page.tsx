...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      error('Sign in failed', err.message || 'Please check your email and password.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleOAuth = async (provider: 'google' | 'github') => {
    try {
      ...
      setOauthNote(
        `${provider.toUpperCase()} OAuth is not enabled in your Supabase dashboard yet. Use Email & Password below to sign in instantly!`
      )
      error(`${provider.toUpperCase()} Not Enabled`, 'Please use Email & Password below, or toggle on the provider in Supabase Dashboard → Authentication → Providers.')
    }
  }
    ...
    <div className="min-h-screen bg-stone-950 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-stone-900/90 border border-stone-800 p-8 rounded-3xl shadow-xl space-y-5">
          {/* Email Sign In Form (Works Out of the Box) */}
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-stone-900 px-2 text-stone-500 font-medium">Or Social Login</span>
            </div>
          </div>
          ...
          >
            ...
            Continue as Guest Member (Sophia)
          </button>
