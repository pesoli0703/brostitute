...
import { paymentService, FLUTTERWAVE_PAYMENT_LINK } from '@/lib/services/payment'
...
function PremiumContent() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (status === 'successful' || searchParams?.get('success') === 'true') {
        try {
          const activated = await paymentService.activateSubscription(currentUserId, 'plan_gold_monthly', 'flutterwave', txRef || undefined)
          setActiveSub(activated)
          confetti({ particleCount: 90, spread: 80, origin: { y: 0.6 } })
          success('Welcome to Brostitute VIP! 🎉', 'Your Flutterwave payment was verified and all premium perks are active.')
        } catch (e: any) {
          console.error(e)
        }
      }
    ...
    load()
  }, [currentUserId, searchParams, success])
  ...
  const handleCheckout = async () => {
    ...
    try {
      if (provider === 'flutterwave') {
        ...
        success('Redirecting to Flutterwave', 'Opening secure checkout in a new window...')
        return
      }

      const result = await paymentService.initializeCheckout({
        profileId: currentUserId,
        email: user?.email || 'member@brostitute.com',
        planId: selectedPlanId,
        provider
      })

      success('Membership Activated! 🎉', `Welcome to Brostitute VIP. All premium features unlocked!`)
      const sub = await paymentService.getUserSubscription(currentUserId)
      setActiveSub(sub)
    } catch (err: any) {
  ...
  return (
    <main className="flex-1 max-w-lg w-full mx-auto px-4 py-6 space-y-6">
      ...
      <div className="p-5 rounded-3xl bg-stone-900 border border-stone-800 space-y-3">
        ...
        {provider === 'flutterwave' && (
          <div className="p-3.5 rounded-2xl bg-amber-950/30 border border-amber-900/50 text-xs text-amber-200 flex items-start gap-2.5">
            <Shield className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />
            <div>
              <p className="font-semibold">Direct Secure Checkout via Flutterwave</p>
              <p className="text-[11px] text-stone-400 mt-0.5">
                Supports Card, Bank Transfer, USSD, and Mobile Money with immediate VIP activation.
              </p>
            </div>
          </div>
        )}
      ...
      <button
        ...
        disabled={loading}
        className="w-full py-4 bg-gradient-to-r from-amber-500 via-rose-500 to-rose-600 hover:from-amber-400 hover:to-rose-500 text-white font-bold rounded-2xl text-sm flex items-center justify-center gap-2 shadow-xl shadow-rose-950/60 transition-all disabled:opacity-50"
      >
        {loading ? (
          'Opening Checkout...'
        ) : provider === 'flutterwave' ? (
...
export default function PremiumPage() {
  return (
    <div className="min-h-screen bg-stone-950 text-stone-100 flex flex-col pb-24 md:pb-8">
      <TopBar />
      <Suspense fallback={<div className="py-12 text-center text-xs text-stone-500">Loading VIP plans...</div>}>
        <PremiumContent />
      </Suspense>
      <MobileNavBar />
    </div>
